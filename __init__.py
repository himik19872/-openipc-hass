...
# Импортируем OpenIPC аудио
try:
    from .openipc_audio import OpenIPCAudio
    OPENIPC_AUDIO_AVAILABLE = True
except ImportError as err:
    OPENIPC_AUDIO_AVAILABLE = False
    OpenIPCAudio = None
    _LOGGER.warning(f"OpenIPC audio module not available: {err}")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    coordinator.openipc_audio = None
    ...
    elif device_type == DEVICE_TYPE_OPENIPC and OPENIPC_AUDIO_AVAILABLE:
        try:
            coordinator.openipc_audio = OpenIPCAudio(
                hass,
                host=entry.data[CONF_HOST],
                username=entry.data[CONF_USERNAME],
                password=entry.data[CONF_PASSWORD]
            )
            # Проверяем доступность аудио эндпоинта
            hass.async_create_task(coordinator.openipc_audio.async_test_connection())
            _LOGGER.info(f"✅ OpenIPC audio initialized for {entry.data.get('name')}")
        except Exception as err:
            _LOGGER.error(f"❌ OpenIPC audio init failed: {err}")
            coordinator.openipc_audio = None
