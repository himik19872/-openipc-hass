...
class OpenIPCVivotekDevice:
    ...
    def __init__(self, hass: HomeAssistant, host: str, username: str, password: str, camera_name: str):
        ...
        # PTZ контроллер (будет инициализирован отдельно)
        self.ptz = None
    ...
    async def async_init_ptz(self):
        """Initialize PTZ control."""
        try:
            # Импортируем здесь, чтобы избежать циклического импорта
            from .vivotek_ptz import VivotekPTZ
            self.ptz = VivotekPTZ(
                self.hass,
                self.host,
                self.username,
            ...
            _LOGGER.info(f"✅ PTZ initialized for Vivotek {self.host}")
            return True
        except Exception as err:
            _LOGGER.error(f"❌ PTZ init failed: {err}")
            self._ptz_available = False
            return False
    ...
    def ptz_available(self) -> bool:
        """Return True if PTZ is available."""
    ...
    def state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ptz_available": self._ptz_available,
        }